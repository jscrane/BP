...
```{r}
recent <- week(max(bp$datetime)) - week(bp$datetime) < 4
...
### Work days
```{r}
plot.blood.pressure(bp[workdays,], "Work")
```

### Holidays
```{r}
plot.blood.pressure(bp[holidays,], "Holidays")
```
...
plot.dates <- function(dates, points, title, col, yl="mmHg") {
  cs <- c("green", "blue")
...
```{r}
plot.dates(dates, bp$pul, "Pulse", "green", "bpm")
```
